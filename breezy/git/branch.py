    ...
    def _update_refs(self, result, old_refs, new_refs, overwrite, tag_selector):
        ...
        def ref_equals(refs, ref, git_sha, revid):
            try:
                ...
                return False
            if value[0] is not None and git_sha is not None and value[0] == git_sha:
                return True
            if value[1] is not None and revid is not None and value[1] == revid:
                return True
        ...
        with self.source.lock_read(), self.target.lock_write():
            ...
            def update_refs(old_refs):
                return self._update_refs(
                    result, old_refs, new_refs, overwrite, tag_selector
                )
