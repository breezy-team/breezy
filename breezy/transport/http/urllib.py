    ...
    def retry_or_raise(self, http_class, request, first_try):
        ...
        if exc_type == socket.gaierror:
            ...
            raise errors.ConnectionError(
                "Couldn't resolve host '%s'" % origin_req_host, orig_error=exc_val
            )
...
class DigestAuthHandler(AbstractAuthHandler):
    ...
    def build_auth_header(self, auth, request):
        ...
        A1 = ("%s:%s:%s" % (auth["user"], auth["realm"], auth["password"])).encode(
            "utf-8"
        )
    ...
    def request(self, method, url, fields=None, headers=None, **urlopen_kw):
        ...
        class Urllib3LikeResponse:
            ...
            def text(self):
                if self.status == 204:
                    return None
                charset = cgi.parse_header(self._actual.headers["Content-Type"])[1].get(
                    "charset"
                )
                if charset:
