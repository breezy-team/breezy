...
class TestDeltaApplication(TestCaseWithTransport):
    ...
    def make_file_ie(self, file_id=b'file-id', name='name', parent_id=None):
        return inventory.InventoryFile(file_id, name, parent_id, b'result', text_size=0, text_sha1=b'')
    ...
    def test_repeated_new_path(self):
        inv = self.get_empty_inventory()
        file1 = inventory.InventoryFile(b'id1', 'path', inv.root.file_id, b'result', text_size=0, text_sha1=b"")
        file2 = inventory.InventoryFile(b'id2', 'path', inv.root.file_id, b'result', text_size=0, text_sha1=b"")
        delta = InventoryDelta(
    ...
    def test_repeated_old_path(self):
        inv = self.get_empty_inventory()
        file1 = inventory.InventoryFile(b'id1', 'path', inv.root.file_id, revision=b'result', text_size=0, text_sha1=b"")
        ...
        file2 = inventory.InventoryFile(b'id2', 'path2', inv.root.file_id, revision=b'result', text_size=0, text_sha1=b"")
        inv.add(file1)
        inv.add(file2)
    ...
    def test_parent_is_not_directory(self):
        inv = self.get_empty_inventory()
        file1 = inventory.InventoryFile(b'id1', 'path', inv.root.file_id, b'result', text_size=0, text_sha1=b"")
        file2 = inventory.InventoryFile(b'id2', 'path2', b'id1', b'result', text_size=0, text_sha1=b"")
        inv.add(file1)
    ...
    def test_parent_is_missing(self):
        inv = self.get_empty_inventory()
        file2 = inventory.InventoryFile(b'id2', 'path2', b'missingparent', revision=b'result', text_size=0, text_sha1=b"")
        delta = InventoryDelta([(None, 'path/path2', b'id2', file2)])
        self.assertRaises(errors.InconsistentDelta, self.apply_delta, self,
                          inv, delta)
    ...
    def test_new_parent_path_has_wrong_id(self):
        ...
        file1 = inventory.InventoryFile(b'id', 'path', b'p-2', b'result', text_size=0,
                                        text_sha1=b"")
        inv.add(parent1)
        inv.add(parent2)
    ...
    def test_old_parent_path_is_wrong(self):
        inv = self.get_empty_inventory()
        parent1 = inventory.InventoryDirectory(b'p-1', 'dir', inv.root.file_id, revision=b'result')
        parent2 = inventory.InventoryDirectory(
            b'p-2', 'dir2', inv.root.file_id, revision=b'result')
        file1 = inventory.InventoryFile(b'id', 'path', b'p-2', revision=b'result', text_size=0, text_sha1=b"")
        inv.add(parent1)
        inv.add(parent2)
        inv.add(file1)
    ...
    def test_old_parent_path_is_for_other_id(self):
        ...
        parent2 = inventory.InventoryDirectory(
            b'p-2', 'dir2', inv.root.file_id, b'result')
        file1 = inventory.InventoryFile(b'id', 'path', b'p-2', b'result', text_size=0, text_sha1=b"")
        file2 = inventory.InventoryFile(b'id2', 'path', b'p-1', b'result', text_size=0, text_sha1=b"")
        inv.add(parent1)
        inv.add(parent2)
        inv.add(file1)
        inv.add(file2)
    ...
    def test_add_file(self):
        inv = self.get_empty_inventory()
        file1 = inventory.InventoryFile(b'file-id', 'path', inv.root.file_id, b'result', text_size=0, text_sha1=b'')
...
class TestInventoryEntry(TestCase):
    ...
    def test_file_detect_changes(self):
        ...
        self.assertEqual((False, False), left.detect_changes(right))
        self.assertEqual((False, False), right.detect_changes(left))
        left = inventory.InventoryFile(b'123', 'hello.c', ROOT_ID, text_sha1=b"123", executable=True)
    ...
    def test_symlink_detect_changes(self):
        ...
        right = inventory.InventoryLink(b'123', 'hello.c', ROOT_ID, symlink_target='foo')
        self.assertEqual((False, False), left.detect_changes(right))
        self.assertEqual((False, False), right.detect_changes(left))
        left = inventory.InventoryLink(b'123', 'hello.c', ROOT_ID, symlink_target='different')
...
class TestDescribeChanges(TestCase):
    ...
    def test_describe_change(self):
        ...
        old_a = InventoryFile(b'a-id', 'a_file', ROOT_ID, text_sha1=b'123132', text_size=0)
        new_a = InventoryFile(b'a-id', 'a_file', ROOT_ID, text_sha1=b'123132', text_size=0)

        self.assertChangeDescription('unchanged', old_a, new_a)

        new_a = InventoryFile(b'a-id', 'a_file', ROOT_ID, text_sha1=b'abcabc', text_size=10)
        ...
        # modification:
        new_a = InventoryFile(b'a-id', 'newfilename', ROOT_ID, text_sha1=b'abcabc', text_size=10)
        self.assertChangeDescription('modified and renamed', old_a, new_a)

        # reparenting is 'renaming'
        new_a = InventoryFile(b'a-id', old_a.name, b'somedir-id', text_sha1=b'abcabc', text_size=10)
        self.assertChangeDescription('modified and renamed', old_a, new_a)

        # reset the content values so its not modified
        new_a = InventoryFile(b'a-id', 'newfilename', b'somedir-id', text_size=old_a.text_size, text_sha1=old_a.text_sha1)
        self.assertChangeDescription('renamed', old_a, new_a)

        # reparenting is 'renaming'
        new_a = InventoryFile(b'a-id', old_a.name, b'somedir-id', text_size=old_a.text_size, text_sha1=old_a.text_sha1)
        self.assertChangeDescription('renamed', old_a, new_a)
...
class TestCHKInventory(tests.TestCaseWithMemoryTransport):
    ...
    def test_id2path(self):
        inv = Inventory(revision_id=b"revid", root_revision=b"rootrev")
        direntry = InventoryDirectory(b"dirid", "dir", inv.root.file_id, revision=b"filerev")
        fileentry = InventoryFile(b"fileid", "file", b"dirid", revision=b"filerev", executable=True, text_sha1=b"ffff", text_size=1)
    ...
    def test_path2id(self):
        ...
        fileentry = InventoryFile(
            b"fileid", "file", b"dirid",
            revision=b"filerev",
            executable=True, text_sha1=b"ffff",
            text_size=1)
        inv.add(direntry)
        inv.add(fileentry)
        chk_bytes = self.get_chk_bytes()
    ...
    def test_create_by_apply_delta_empty_add_child(self):
        ...
        chk_bytes = self.get_chk_bytes()
        base_inv = CHKInventory.from_inventory(chk_bytes, inv)
        a_entry = InventoryFile(b"A-id", "A", inv.root.file_id, revision=b"filerev", executable=True, text_sha1=b"ffff", text_size=1)
        inv.add(a_entry)
        inv.revision_id = b"expectedid"
        reference_inv = CHKInventory.from_inventory(chk_bytes, inv)
    ...
    def test_iter_changes(self):
        ...
        inv2.add(InventoryFile(b"fileid", "file", inv.root.file_id, revision=b"filerev2", executable=False, text_sha1=b"bbbb", text_size=2))
        # get fresh objects.
        chk_bytes = self.get_chk_bytes()
        chk_inv = CHKInventory.from_inventory(chk_bytes, inv)
        lines = chk_inv.to_lines()
    ...
    def test_file_entry_to_bytes(self):
        CHKInventory(None)
        ie = inventory.InventoryFile(b'file-id', 'filename', b'parent-id', executable=True, revision=b'file-rev-id', text_sha1=b'abcdefgh', text_size=100)
    ...
    def test_file2_entry_to_bytes(self):
        CHKInventory(None)
        # \u30a9 == 'omega'
        ie = inventory.InventoryFile(b'file-id', '\u03a9name', b'parent-id', executable=False, revision=b'file-rev-id', text_sha1=b'123456', text_size=25)
        bytes = _chk_inventory_entry_to_bytes(ie)
        self.assertEqual(b'file: file-id\nparent-id\n\xce\xa9name\n'
                         b'file-rev-id\n123456\n25\nN', bytes)
        ie2 = _chk_inventory_bytes_to_entry(bytes)
    ...
    def test_dir_entry_to_bytes(self):
        CHKInventory(None)
        ie = inventory.InventoryDirectory(b'dir-id', 'dirname', b'parent-id', revision=b'dir-rev-id')
        bytes = _chk_inventory_entry_to_bytes(ie)
    ...
    def test_dir2_entry_to_bytes(self):
        CHKInventory(None)
        ie = inventory.InventoryDirectory(b'dir-id', 'dir\u03a9name', b'pid', revision=b'dir-rev-id')
        bytes = _chk_inventory_entry_to_bytes(ie)
        self.assertEqual(b'dir: dir-id\npid\ndir\xce\xa9name\n'
                         b'dir-rev-id', bytes)
        ie2 = _chk_inventory_bytes_to_entry(bytes)
    ...
    def test_symlink_entry_to_bytes(self):
        CHKInventory(None)
        ie = inventory.InventoryLink(b'link-id', 'linkname', b'parent-id', revision=b'link-rev-id', symlink_target='target/path')
        bytes = _chk_inventory_entry_to_bytes(ie)
    ...
    def test_symlink2_entry_to_bytes(self):
        ...
        ie = inventory.InventoryLink(
            b'link-id', 'link\u03a9name', b'parent-id', revision=b'link-rev-id', symlink_target='target/\u03a9path')
        bytes = _chk_inventory_entry_to_bytes(ie)
        self.assertEqual(b'symlink: link-id\nparent-id\nlink\xce\xa9name\n'
    ...
    def test_tree_reference_entry_to_bytes(self):
        ...
        ie = inventory.TreeReference(b'tree-root-id', 'tree\u03a9name',
                                     b'parent-id', revision=b'tree-rev-id',
    ...
    def make_basic_utf8_inventory(self):
        ...
        inv.add(InventoryFile(b"fileid", 'f\xefle', root_id, revision=b"filerev", text_sha1=b"ffff", text_size=0))
        inv.add(InventoryDirectory(b"dirid", 'dir-\N{EURO SIGN}', root_id, revision=b"dirrev"))
        inv.add(InventoryFile(b"childid", 'ch\xefld', b"dirid", revision=b"filerev", text_sha1=b"ffff", text_size=0))
        chk_bytes = self.get_chk_bytes()
        chk_inv = CHKInventory.from_inventory(chk_bytes, inv)
        lines = chk_inv.to_lines()
    ...
    def test__preload_populates_cache(self):
        inv = Inventory(revision_id=b"revid", root_revision=b"rootrev")
        root_id = inv.root.file_id
        inv.add(InventoryFile(b"fileid", "file", root_id, revision=b"filerev", executable=True, text_sha1=b"ffff", text_size=1))
        inv.add(InventoryDirectory(b"dirid", "dir", root_id, revision=b"dirrev"))
        inv.add(InventoryFile(b"childid", "child", b"dirid", revision=b"filerev", executable=False, text_sha1=b"dddd", text_size=1))
        chk_bytes = self.get_chk_bytes()
        chk_inv = CHKInventory.from_inventory(chk_bytes, inv)
        lines = chk_inv.to_lines()
        new_inv = CHKInventory.deserialise(chk_bytes, lines, (b"revid",))
...
class TestCHKInventoryExpand(tests.TestCaseWithMemoryTransport):
    ...
    def make_dir(self, inv, name, parent_id, revision):
        ie = inv.make_entry('directory', name, parent_id, name.encode('utf-8') + b'-id', revision=revision)
        inv.add(ie)

    def make_file(self, inv, name, parent_id, revision, content=b'content\n'):
        ie = inv.make_entry('file', name, parent_id,
                            name.encode('utf-8') + b'-id', text_sha1=osutils.sha_string(content), text_size=len(content), revision=revision)
        inv.add(ie)
