...
def _replace_entry(inv, file_id, **updates):
    """Replace inv[file_id] with a copy that has the given attrs updated."""
    old = inv.get_entry(file_id)
    kwargs = {
        "file_id": old.file_id,
        "name": old.name,
    ...
    if old.kind == "file":
        kwargs["text_sha1"] = old.text_sha1
        kwargs["text_size"] = old.text_size
        kwargs["executable"] = old.executable
        cls = InventoryFile
    elif old.kind == "directory":
        cls = InventoryDirectory
    elif old.kind == "symlink":
        kwargs["symlink_target"] = old.symlink_target
        cls = inventory.InventoryLink
    elif old.kind == "tree-reference":
        kwargs["reference_revision"] = old.reference_revision
        cls = TreeReference
    else:
        raise AssertionError(f"unknown kind {old.kind}")
...
class TestInventoryUpdates(TestCase):
    ...
    def test_copy_copies_root_revision(self):
        ...
        _set_root_revision(inv, b"therev")
        inv2 = inv.copy()
...
class TestDeltaApplication(TestCaseWithTransport):
    ...
    def get_empty_inventory(self, reference_inv=None):
        ...
        if reference_inv is not None:
            _set_root_revision(inv, reference_inv.root.revision)
        else:
            _set_root_revision(inv, b"basis")
        return inv

    def make_file_ie(self, file_id=b"file-id", name="name", parent_id=None):
        ie_file = inventory.InventoryFile(
            file_id=file_id,
            name=name,
            parent_id=parent_id,
            revision=b"result",
    ...
    def test_empty_delta(self):
        ...
        self.assertEqual(
            list(inv.iter_entries_by_dir()),
            list(inv2.iter_entries_by_dir()),
        )
    ...
    def test_repeated_file_id(self):
        ...
        self.assertRaises(BzrInconsistentDelta, self.apply_delta, self, inv, delta)
    ...
    def test_remove_dir_leaving_dangling_child(self):
        ...
        dir3 = inventory.InventoryDirectory(
            file_id=b"p-3",
            name="child2",
            parent_id=b"p-1",
            revision=b"result",
...
class TestInventoryEntry(TestCase):
    ...
    def test_file_detect_changes(self):
        def mkfile(**overrides):
            ...
            kw.update(overrides)
            return inventory.InventoryFile(**kw)

        left = mkfile()
        right = mkfile()
        self.assertEqual((False, False), left.detect_changes(right))
        self.assertEqual((False, False), right.detect_changes(left))
        left = mkfile(executable=True)
        self.assertEqual((False, True), left.detect_changes(right))
        self.assertEqual((False, True), right.detect_changes(left))
        right = mkfile(text_sha1=b"321")
        self.assertEqual((True, True), left.detect_changes(right))
        self.assertEqual((True, True), right.detect_changes(left))

    def test_symlink_detect_changes(self):
        def mklink(**overrides):
            kw = {
                "file_id": b"123",
                "name": "hello.c",
                "parent_id": ROOT_ID,
            ...
            kw.update(overrides)
            return inventory.InventoryLink(**kw)

        left = mklink()
        right = mklink()
        self.assertEqual((False, False), left.detect_changes(right))
        self.assertEqual((False, False), right.detect_changes(left))
        left = mklink(symlink_target="different")
...
class TestDescribeChanges(TestCase):
    def test_describe_change(self):
        ...
        def mkfile(**overrides):
            ...
            kw.update(overrides)
            return InventoryFile(**kw)

        old_a = mkfile()
        new_a = mkfile()
        self.assertChangeDescription("unchanged", old_a, new_a)

        new_a = mkfile(text_size=10, text_sha1=b"abcabc")
        ...
        # modification:
        new_a = mkfile(text_size=10, text_sha1=b"abcabc", name="newfilename")
        self.assertChangeDescription("modified and renamed", old_a, new_a)

        # reparenting is 'renaming'
        new_a = mkfile(text_size=10, text_sha1=b"abcabc", parent_id=b"somedir-id")
        self.assertChangeDescription("modified and renamed", old_a, new_a)

        # reset the content values so its not modified, just renamed
        new_a = mkfile(name="newfilename")
        self.assertChangeDescription("renamed", old_a, new_a)

        # reparenting is 'renaming'
        new_a = mkfile(parent_id=b"somedir-id")
        self.assertChangeDescription("renamed", old_a, new_a)
...
class TestCHKInventory(tests.TestCaseWithMemoryTransport):
    ...
    def test_deserialise_gives_CHKInventory(self):
        ...
        _set_root_revision(inv, b"rootrev")
    ...
    def test_directory_children_on_demand(self):
        ...
        lines = chk_inv.to_lines()
        new_inv = CHKInventory.deserialise(chk_bytes, lines, (b"revid",))
        children = new_inv.get_children(inv.root.file_id)
        self.assertEqual({"file"}, set(children))
        file_direct = new_inv.get_entry(b"fileid")
        file_found = children["file"]
    ...
    def test_id2path(self):
        ...
        _replace_entry(inv, b"dirid", revision=b"filerev")
        chk_bytes = self.get_chk_bytes()
        chk_inv = CHKInventory.from_inventory(chk_bytes, inv)
        lines = chk_inv.to_lines()
        new_inv = CHKInventory.deserialise(chk_bytes, lines, (b"revid",))
    ...
    def test_iter_changes(self):
        ...
        _set_root_revision(inv2, b"rootrev")
    ...
    def make_basic_utf8_inventory(self):
        ...
        _replace_entry(
            inv, b"fileid", revision=b"filerev", text_sha1=b"ffff", text_size=0
        )
        inv.add(InventoryDirectory(b"dirid", "dir-\N{EURO SIGN}", root_id))
        _replace_entry(inv, b"dirid", revision=b"dirrev")
        inv.add(InventoryFile(b"childid", "ch\xefld", b"dirid"))
        _replace_entry(
            inv, b"childid", revision=b"filerev", text_sha1=b"ffff", text_size=0
        )
        chk_bytes = self.get_chk_bytes()
        chk_inv = CHKInventory.from_inventory(chk_bytes, inv)
        lines = chk_inv.to_lines()
    ...
    def test__preload_handles_utf8(self):
        ...
        self.assertEqual(
            ...
            sorted(new_inv.get_children(new_inv.root_id)),
        )
        self.assertEqual(["ch\xefld"], sorted(new_inv.get_children(b"dirid")))
    ...
    def test__preload_populates_cache(self):
        ...
        self.assertEqual(["dir", "file"], sorted(new_inv.get_children(root_id)))
        self.assertEqual(["child"], sorted(new_inv.get_children(b"dirid")))

    def test__preload_handles_partially_evaluated_inventory(self):
        new_inv = self.make_basic_utf8_inventory()
        # Children are reachable via get_children both before and after
        # _preload_cache; bzrformats no longer exposes a lazy ._children
    ...
    def test_filter_change_in_renamed_subfolder(self):
        ...
        _set_root_revision(inv, b"rootrev")
        src_ie = InventoryDirectory(
            file_id=b"src-id",
            name="src",
            parent_id=b"tree-root",
            revision=b"srcrev",
        )
        inv.add(src_ie)
        sub_ie = InventoryDirectory(
            file_id=b"sub-id",
            name="sub",
            parent_id=b"src-id",
            revision=b"subrev",
...
class TestMutableInventoryFromTree(TestCaseWithTransport):
    ...
    def test_some_files(self):
        ...
        _replace_entry(inv, b"thefileid", executable=True)
        self.assertFalse(tree.root_inventory.get_entry(b"thefileid").executable)
