...
class BundleInfo:
    ...
    def _update_tree(self, bundle_tree, revision_id):
        ...
        def renamed(kind, extra, lines):
            ...
            if len(info) < 2:
                raise BzrError(
                    "renamed action lines need both a from and to" ": {!r}".format(
                        extra
                    )
                )
            old_path = info[0]
        ...
        def removed(kind, extra, lines):
            ...
            if len(info) > 1:
                ...
                raise BzrError(
                    "removed action lines should only have the path" ": {!r}".format(
                        extra
                    )
                )
            path = info[0]
        ...
        def added(kind, extra, lines):
            ...
            if len(info) <= 1:
                raise BzrError(
                    "add action lines require the path and file id" ": {!r}".format(
                        extra
                    )
                )
            elif len(info) > 5:
            ...
            if not info[1].startswith("file-id:"):
                raise BzrError(
                    "The file-id should follow the path for an add" ": {!r}".format(
                        extra
                    )
                )
        ...
        def modified(kind, extra, lines):
            ...
            if len(info) < 1:
                raise BzrError(
                    "modified action lines have at least"
                    "the path in them: {!r}".format(extra)
                )
            path = info[0]
        ...
        for action_line, lines in self.get_revision_info(revision_id).tree_actions:
            ...
            if second == -1:
                raise BzrError(
                    "Bogus action line" " (missing second space): {!r}".format(
                        action_line
                    )
                )
            ...
            if action not in valid_actions:
                raise BzrError(
                    "Bogus action line" " (unrecognized action): {!r}".format(
                        action_line
                    )
                )
