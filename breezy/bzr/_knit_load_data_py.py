...
def _load_data_py(kndx, fp):
    ...
    for line in fp.readlines():
        ...
        try:
            ...
            # one, so we error.
            raise KnitCorrupt(kndx._filename, f"line {rec!r}: {e}")

        version_id, options, pos, size = rec[:4]
        try:
            pos = int(pos)
        except ValueError as e:
            raise KnitCorrupt(kndx._filename, f"invalid position on line {rec!r}: {e}")
        try:
            size = int(size)
        except ValueError as e:
            raise KnitCorrupt(kndx._filename, f"invalid size on line {rec!r}: {e}")
