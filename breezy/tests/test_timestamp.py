...
class TestPatchHeader(tests.TestCase):
    ...
    def test_format_patch_date(self):
        ...
        self.assertEqual('1970-01-01 00:00:00 +0000',
                         patch.format_patch_date(0))
        self.assertEqual('1970-01-01 00:00:00 +0000',
                         patch.format_patch_date(0, 5 * 3600))
        self.assertEqual('1970-01-01 00:00:00 +0000',
                         patch.format_patch_date(0, -5 * 3600))
        # regular timestamp with typical timezone
        self.assertEqual('2007-03-06 10:04:19 -0500',
                         patch.format_patch_date(1173193459, -5 * 3600))
        ...
        self.assertEqual('2007-03-06 15:05:19 +0001',
                         patch.format_patch_date(1173193459, +1 * 60))

    def test_parse_patch_date(self):
        self.assertEqual(
            (0, 0),
            patch.parse_patch_date('1970-01-01 00:00:00 +0000'))
        # even though we don't emit pre-epoch dates, we can parse them
        self.assertEqual(
            (0, -5 * 3600),
            patch.parse_patch_date('1969-12-31 19:00:00 -0500'))
        self.assertEqual(
            (0, +5 * 3600),
            patch.parse_patch_date('1970-01-01 05:00:00 +0500'))
        self.assertEqual(
            (1173193459, -5 * 3600),
        ...
        self.assertEqual(
            (1173193459, +3 * 60),
            patch.parse_patch_date('2007-03-06 15:07:19 +0003'))
    ...
    def test_parse_patch_date_bad(self):
        self.assertRaises(ValueError, patch.parse_patch_date,
                          'NOT A TIME')
        # Extra data at end
        self.assertRaises(ValueError, patch.parse_patch_date,
...
class FormatHighresDateTests(tests.TestCase):
    ...
    def test_format_highres_date(self):
        ...
        self.assertEqual(format_highres_date(1120153132.350850105, 0), 'Thu 2005-06-30 17:38:52.350850105 +0000')
        self.assertEqual(format_date(1120153132.350850105, -5 * 3600), 'Thu 2005-06-30 12:38:52 -0500')
        self.assertEqual(format_highres_date(1120153132.350850105, -5 * 3600), 'Thu 2005-06-30 12:38:52.350850105 -0500')
        self.assertEqual(format_highres_date(1120153132.350850105, 7200), 'Thu 2005-06-30 19:38:52.350850105 +0200')
        self.assertEqual(format_highres_date(1152428738.867522, 19800), 'Sun 2006-07-09 12:35:38.867522001 +0530')
...
class UnpackHighresDateTests(tests.TestCase):
    ...
    def test_random(self):
        t = time.time()
        o = local_time_offset()
        t2, o2 = unpack_highres_date(
            format_highres_date(t, o))
