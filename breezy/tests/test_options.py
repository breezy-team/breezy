...
class OptionTests(TestCase):
    ...
    def test_registry_conversion(self):
        ...
        bzr.register_metadir(
            registry, "hidden", "RepositoryFormatKnit1", "two help", hidden=True
        )
    ...
    def test_from_kwargs(self):
        my_option = option.RegistryOption.from_kwargs(
            "my-option", help="test option", short="be short", be_long="go long"
        )
        ...
        self.assertEqual(
            ["my-option", "be-long", "short"], [x[0] for x in my_option.iter_switches()]
        )
    ...
    def test_help(self):
        ...
        bzr.register_metadir(
            registry, "hidden", "RepositoryFormat7", "hidden help", hidden=True
        )
        registry.set_default("one")
        options = [
            option.RegistryOption(
    ...
    def test_iter_switches(self):
        ...
        opt = option.RegistryOption(
            "format", "format help", registry, value_switches=False
        )
        self.assertEqual(
            list(opt.iter_switches()), [("format", None, "ARG", "format help")]
        )
        opt = option.RegistryOption(
            "format", "format help", registry, value_switches=True
        )
        self.assertEqual(
            list(opt.iter_switches()),
            [
    ...
    def test_option_grammar(self):
        ...
        for scope, opt in self.get_builtin_command_options():
            for name, _, _, helptxt in opt.iter_switches():
                ...
                if not helptxt:
                    msgs.append(
                        "%-16s %-16s %s" % ((scope or "GLOBAL"), name, "NO HELP")
                    )
...
class TestOptionMisc(TestCase):
    def test_is_hidden(self):
        registry = controldir.ControlDirFormatRegistry()
        bzr.register_metadir(
            registry, "hidden", "HiddenFormat", "hidden help text", hidden=True
        )
        bzr.register_metadir(
            registry, "visible", "VisibleFormat", "visible help text", hidden=False
        )
    ...
    def test_short_name(self):
        ...
        opt = option.RegistryOption(
            "format", short_name="F", help="", registry=registry
        )
