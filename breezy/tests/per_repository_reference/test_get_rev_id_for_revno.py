...
class TestGetRevIdForRevno(TestCaseWithExternalReferenceRepository):
    def test_uses_fallback(self):
        ...
        branch = spare_tree.branch.create_clone_on_transport(
            self.get_transport("referring"), stacked_on=tree.branch.base
        )
        repo = branch.repository
        # Sanity check: now repo has 'revid3', and base has 'revid' + 'revid2'
        self.assertEqual(
            {revid3}, set(repo.controldir.open_repository().all_revision_ids())
        )
        self.assertEqual(
            {revid2, revid}, set(base.controldir.open_repository().all_revision_ids())
        )
