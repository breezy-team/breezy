...
class TestTreeTransform(TestCaseWithWorkingTree):
    ...
    def test_existing_limbo(self):
        ...
        transform, _root = self.transform()
        transform.apply()
    ...
    def test_build(self):
        ...
        self.assertRaises(
            DuplicateKey, transform.version_file, trans_id, file_id=b"my_pretties"
        )
        if self.wt.supports_setting_file_ids():
        ...
        if self.wt.supports_setting_file_ids():
            tree_mod_paths = [
                self.wt.abspath(self.wt.id2path(f))
                for f in (b"ozzie", b"my_pretties", b"my_pretties2")
            ]
    ...
    def test_apply_informs_tree_of_observed_sha1(self):
        ...
        trans_id = trans.new_file(
            "file1", root, contents, file_id=b"file1-id", sha1=sha1
        )
        calls = []
    ...
    def test_tree_reference(self):
        ...
        self.assertEqual(nested_revid, tree.get_reference_revision("reference"))

    def test_conflicts(self):
        transform, root = self.transform()
        trans_id = transform.new_file("name", root, [b"contents"], b"my_pretties")
        self.assertEqual(len(transform.find_raw_conflicts()), 0)
        trans_id2 = transform.new_file("name", root, [b"Crontents"], b"toto")
        self.assertEqual(
            transform.find_raw_conflicts(), [("duplicate", trans_id, trans_id2, "name")]
        )
        self.assertRaises(MalformedTransform, transform.apply)
        transform.adjust_path("name", trans_id, trans_id2)
        self.assertEqual(
            transform.find_raw_conflicts(), [("non-directory parent", trans_id)]
        )
        tinman_id = transform.trans_id_tree_path("tinman")
        transform.adjust_path("name", tinman_id, trans_id2)
        if self.wt.has_versioned_directories():
            self.assertEqual(
                transform.find_raw_conflicts(),
                [("unversioned parent", tinman_id), ("missing parent", tinman_id)],
            )
        else:
            self.assertEqual(
                transform.find_raw_conflicts(), [("missing parent", tinman_id)]
            )
        ...
        if self.wt.has_versioned_directories():
            self.assertEqual(
                transform.find_raw_conflicts(),
                [("unversioned parent", lion_id), ("missing parent", lion_id)],
            )
        else:
            self.assertEqual(
                transform.find_raw_conflicts(), [("missing parent", lion_id)]
            )
        transform.version_file(lion_id, file_id=b"Courage")
        self.assertEqual(
            transform.find_raw_conflicts(),
            [("missing parent", lion_id), ("versioning no contents", lion_id)],
        )
        transform.adjust_path("name2", root, trans_id2)
        self.assertEqual(
            transform.find_raw_conflicts(), [("versioning no contents", lion_id)]
        )
        transform.create_file([b"Contents, okay?"], lion_id)
        transform.adjust_path("name2", trans_id2, trans_id2)
        self.assertEqual(
            transform.find_raw_conflicts(),
            [("parent loop", trans_id2), ("non-directory parent", trans_id2)],
        )
        transform.adjust_path("name2", root, trans_id2)
        oz_id = transform.new_directory("oz", root)
        transform.set_executability(True, oz_id)
        self.assertEqual(
            transform.find_raw_conflicts(), [("unversioned executability", oz_id)]
        )
        transform.version_file(oz_id, file_id=b"oz-id")
        self.assertEqual(
            transform.find_raw_conflicts(), [("non-file executability", oz_id)]
        )
        ...
        if self.wt.supports_setting_file_ids():
            ...
            self.assertEqual((result[1][0], result[1][2]), ("duplicate id", newtip))
        else:
            self.assertEqual(len(result), 1)
        self.assertEqual((result[0][0], result[0][1]), ("duplicate", newtip))
        transform2.finalize()
        transform3 = self.wt.transform()
        self.addCleanup(transform3.finalize)
        oz_id = transform3.trans_id_tree_path("oz")
        transform3.delete_contents(oz_id)
        self.assertEqual(transform3.find_raw_conflicts(), [("missing parent", oz_id)])
    ...
    def test_adjust_path_updates_child_limbo_names(self):
        ...
        self.assertStartsWith(
            transform._limbo_name(qux_id), transform._limbo_name(bar_id)
        )
    ...
    def test_unversioning(self):
        ...
        if self.wt.has_versioned_directories():
            self.assertEqual(
                unversion.find_raw_conflicts(), [("unversioned parent", parent_id)]
            )
        else:
    ...
    def test_both_rename2(self):
        ...
        create_tree.new_file(
            "test_too_much.py", blackbox, [b"hello1"], b"test_too_much-id"
        )
        create_tree.apply()
        mangle_tree, root = self.transform()
    ...
    def test_both_rename3(self):
        ...
        create_tree.new_file(
            "test_too_much.py", tests, [b"hello1"], b"test_too_much-id"
        )
        create_tree.apply()
        mangle_tree, root = self.transform()
    ...
    def _test_symlinks(self, link_name1, link_target1, link_name2, link_target2):
        ...
        self.assertEqual(
            transform.find_raw_conflicts(), [("non-file executability", wiz_id)]
        )
        ...
        self.assertEqual(
            link_target2, osutils.readlink(self.wt.abspath(ozpath(link_name2)))
        )
        self.assertEqual(
    ...
    def test_symlinks(self):
        self._test_symlinks("wizard", "wizard-target", "wizard2", "behind_curtain")
    ...
    def get_conflicted(self):
        ...
        new_dorothy = conflicts.new_file("dorothy", root, [b"dorothy"], b"dorothy-id")
    ...
    def test_conflict_resolution(self):
        conflicts, emerald, oz, old_dorothy, new_dorothy, _munchkincity = (
            self.get_conflicted()
        )
    ...
    def test_cook_conflicts(self):
        ...
        if self.wt.supports_setting_file_ids():
            ...
            missing_parent = MissingParent(
                "Created directory", "munchkincity", b"munchkincity-id"
            )
            deleted_parent = DeletingParent("Not deleting", "oz", b"oz-id")
            self.assertEqual(cooked_conflicts[2], missing_parent)
            unversioned_parent = UnversionedParent(
                "Versioned directory", "munchkincity", b"munchkincity-id"
            )
            unversioned_parent2 = UnversionedParent(
                "Versioned directory", "oz", b"oz-id"
            )
    ...
    def test_string_conflicts(self):
        ...
        if self.wt.supports_setting_file_ids():
            ...
            self.assertEqual(
                conflicts_s[3],
                "Conflict because munchkincity is not"
    ...
    def test_resolve_conflicts_wrong_existing_parent_kind(self):
        ...
        raw_conflicts = resolve_conflicts(tt)
        self.assertEqual(
            {("non-directory parent", "Created directory", "new-3")}, raw_conflicts
        )
        cooked_conflicts = list(tt.cook_conflicts(raw_conflicts))
        from ...bzr.workingtree import InventoryWorkingTree
        ...
        if isinstance(tt._tree, InventoryWorkingTree):
            self.assertEqual(
                [NonDirectoryParent("Created directory", "parent.new", b"parent-id")],
                cooked_conflicts,
            )
        else:
    ...
    def test_resolve_conflicts_missing_parent(self):
        ...
        self.assertEqual(
            ("missing parent", "Created directory", "new-1"), raw_conflicts.pop()
        )
    ...
    def test_renames(self):
        ...
        myfile = create.new_file("myfile", intermediate, [b"myfile-text"], b"myfile-id")
        create.apply()
    ...
    def test_set_executability_order(self):
        ...
        sac = transform.new_file(
            "set_after_creation", root, [b"Set after creation"], b"sac"
        )
    ...
    def test__set_mode_stats_correctly(self):
        ...
        bar1_id = transform.new_file(
            "bar", root, [b"bar contents 1\n"], file_id=b"bar-id-1", executable=False
        )
        transform.apply()
    ...
    def assertTreeChanges(self, tt, expected):
        ...
        self.assertThat(
            actual, MatchesTreeChanges(tt._tree.basis_tree(), tt._tree, expected)
        )
    ...
    def test_rename_count(self):
        ...
        transform2, root = self.transform()
        transform2.adjust_path("name2", root, transform2.trans_id_tree_path("name1"))
