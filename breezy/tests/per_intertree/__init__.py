...
class TestCaseWithTwoTrees(TestCaseWithTree):
    ...
    def make_to_branch_and_tree(self, relpath):
        ...
        made_control = self.make_controldir(
            relpath, format=self.workingtree_format_to._matchingcontroldir
        )
...
def mutable_trees_to_revision_trees(test_case, source, target):
    ...
    return (
        revision_tree_from_workingtree(test_case, source),
...
def load_tests(loader, standard_tests, pattern):
    ...
    for optimiser in InterTree.iter_optimisers():
        if optimiser is inventorytree.InterCHKRevisionTree:
            ...
            optimiser._matching_from_tree_format is not None
