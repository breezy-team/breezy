...
class TestPush(tests.TestCaseWithTransport):
    def test_push_error_on_vfs_http(self):
        ...
        self.run_bzr_error(
            ["http does not support mkdir"], ["push", public_url], working_dir="source"
        )
    ...
    def test_push_remember(self):
        ...
        self.assertEqual(
            osutils.abspath(branch_a.get_push_location()),
    ...
    def test_push_new_branch_stacked_on(self):
        ...
        out, err = self.run_bzr(
            ["push", "--stacked-on", trunk_tree.branch.base, self.get_url("published")],
            working_dir="branch",
        )
        self.assertEqual("", out)
        self.assertEqual(
    ...
    def test_push_new_branch_stacked_on_can_use_dir_urls(self):
        ...
        # we publish branch_tree with a reference to the mainline.
        out, err = self.run_bzr(
            ["push", "--stacked-on", "foo:", self.get_url("published")],
            working_dir="branch",
        )
        self.assertEqual("", out)
        self.assertEqual(
    ...
    def test_push_new_branch_stacked_uses_parent_when_no_public_url(self):
        ...
        # for us.
        out, err = self.run_bzr(
            ["push", "--stacked", self.get_url("published")], working_dir="branch"
        )
        self.assertEqual("", out)
        self.assertEqual(
            "Created new stacked branch referring to {}.\n".format(
...
class TestPushRedirect(tests.TestCaseWithTransport):
    ...
    def test_push_redirects_on_mkdir(self):
        ...
        remote_revision = branch.Branch.open(
            self.memory_server.get_url() + "target"
        ).last_revision()
