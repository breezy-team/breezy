...
""",
    ...
    def test_diff_illegal_prefix_value(self):
        ...
        _out, err = self.run_bzr("diff --color=never --prefix old/", retcode=3)
...
""",
    ...
    def test_diff_nonexistent_dotted_revision(self):
        _out, err = self.run_bzr("diff --color=never -r 1.1", retcode=3)
        self.assertContainsRe(
            err, "Requested revision: '1.1' does not exist in branch:"
        )

    def test_diff_nonexistent_dotted_revision_change(self):
        _out, err = self.run_bzr("diff --color=never -c 1.1", retcode=3)
        self.assertContainsRe(
            err, "Requested revision: '1.1' does not exist in branch:"
        )
    ...
    def test_diff_unversioned(self):
        ...
        _out, err = self.run_bzr("diff --color=never unversioned-file", retcode=3)
    ...
    def test_moved_away(self):
        ...
        tree.add("a")
        output, _error = self.run_bzr("diff --color=never -p0", retcode=1)
        self.assertEqualDiff(
            """\
=== added file 'a'
