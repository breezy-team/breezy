...
''')
    ...
    def test_diff_nonexistent(self):
        ...
        # (Malone #3619)
        self.make_example_branch()
        out, err = self.run_bzr('diff --color=never does-not-exist', retcode=3,
                                error_regexes=('not versioned.*does-not-exist',))

    def test_diff_illegal_revision_specifiers(self):
        out, err = self.run_bzr('diff --color=never -r 1..23..123', retcode=3,
                                error_regexes=('one or two revision specifiers',))

    def test_diff_using_and_format(self):
        out, err = self.run_bzr('diff --color=never --format=default --using=mydi', retcode=3,
                                error_regexes=('are mutually exclusive',))

    def test_diff_nonexistent_revision(self):
        out, err = self.run_bzr('diff --color=never -r 123', retcode=3,
    ...
    def test_diff_revno_branches(self):
        ...
        out, err = self.run_bzr('diff --color=never -r revno:1:branch2..revno:1:branch1',
                                )
        self.assertEqual('', err)
        self.assertEqual('', out)
        out, err = self.run_bzr('diff --color=never -r revno:2:branch2..revno:1:branch1',
                                retcode=1)
        self.assertEqual('', err)
        self.assertEqualDiff("=== modified file 'file'\n"
                             "--- old/file\tYYYY-MM-DD HH:MM:SS +ZZZZ\n"
    ...
    def test_diff_to_working_tree_in_subdir(self):
        ...
        output = self.run_bzr('diff --color=never -r 1..', retcode=1,
...
class TestExternalDiff(DiffBase):
    ...
    def test_external_diff_options_and_using(self):
        ...
        out, err = self.run_bzr('diff --color=never --diff-options -i --diff-options -a --using diff',
                                retcode=1)
