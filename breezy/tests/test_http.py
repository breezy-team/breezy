...
def vary_by_http_auth_scheme():
    scenarios = [
        ...
        ("basicdigest", dict(_auth_server=http_utils.HTTPBasicAndDigestAuthServer)),
    ]
...
class TestHTTPConnections(http_utils.TestCaseWithWebserver):
    ...
    def test_http_get(self):
        ...
        self.assertTrue(
            server.logs[0].find(
                '"GET /foo/bar HTTP/1.1" 200 - "-" "Breezy/%s' % breezy.__version__
            )
            > -1
        )
...
class TestPost(tests.TestCase):
    ...
    def test_post_body_is_received(self):
        ...
        self.assertTrue(
            b"content-type: application/octet-stream\r" in server.received_bytes.lower()
        )
...
class TestWallServer(TestSpecificRequestHandler):
    ...
    def test_http_has(self):
        ...
        self.assertRaises(
            (errors.ConnectionError, errors.InvalidHttpResponse), t.has, "foo/bar"
        )
...
class TestAuth(http_utils.TestCaseWithWebserver):
    ...
    def test_no_credential_leaks_in_log(self):
        ...
        sent_auth_headers = [
            line
            for line in self.mutters
            if line.startswith("> {}".format(self._auth_header))
        ]
        self.assertLength(1, sent_auth_headers)
        self.assertStartsWith(
            sent_auth_headers[0], "> {}: <masked>".format(self._auth_header)
        )
...
class SmartHTTPTunnellingTest(tests.TestCaseWithTransport):
    ...
    def test_smart_http_server_post_request_handler(self):
        ...
        # address, so far...
        request_handler = http_utils.SmartRequestHandler(
