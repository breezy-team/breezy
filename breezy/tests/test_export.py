...
class TestDirExport(tests.TestCaseWithTransport):
    ...
    def test_files_per_file_timestamps(self):
        ...
        builder.build_snapshot(
            None, [("add", ("b", b"b-id", "file", b"content\n"))], timestamp=b_time
        )
        builder.finish_series()
        b = builder.get_branch()
        b.lock_read()
    ...
    def test_subdir_files_per_timestamps(self):
        ...
        tree = b.basis_tree()
        export.export(
            tree, "target", format="dir", subdir="subdir", per_file_timestamps=True
        )
        t = self.get_transport("target")
...
class TarExporterTests(tests.TestCaseWithTransport):
    ...
    def test_tgz_ignores_dest_path(self):
        ...
        os.mkdir("testdir2")
        export.export(
            wt, "testdir1/target.tar.gz", format="tgz", per_file_timestamps=True
        )
        export.export(
            wt, "testdir2/target.tar.gz", format="tgz", per_file_timestamps=True
        )
...
class ZipExporterTests(tests.TestCaseWithTransport):
    def test_per_file_timestamps(self):
        ...
        export.export(
            tree.basis_tree(), "test.zip", format="zip", per_file_timestamps=True
        )
