...
class MergePackageTests(TestCaseWithTransport):
    ...
    def test_debian_upstream_newer(self):
        ...
        conflicts = ubup.merge_from_branch(debp.branch, to_revision=self.revid_debp_n_C)
    ...
    def test_deb_upstream_conflicts_with_ubu_packaging(self):
        ...
        its_parents_map = ubup.branch.repository.get_parent_map([merged_parent])
    ...
    def test_debian_upstream_older(self):
        ...
        # Attempt a plain merge first.
        conflicts = ubup.merge_from_branch(debp.branch, to_revision=self.revid_debp_o_C)
    ...
    def _setup_debian_upstream_older(self):
        ...
        ubuu_n = debu_o.controldir.sprout(
            name, revision_id=self.revid_debu_o_B
        ).open_workingtree()
        ...
        ubup_n = debu_o.controldir.sprout(
            name, revision_id=self.revid_debu_o_A
        ).open_workingtree()
    ...
    def _setup_upstreams_not_diverged(self):
        ...
        ubuntup = upstream.controldir.sprout(
            name, revision_id=self.revid_upstream_A
        ).open_workingtree()
