...
class TestDo(ExternalBase):
    ...
    def build_dir(self):
        return os.path.join(
            "..", "build-area", self.package_name + "-" + str(self.package_version)
        )
    ...
    def test_bd_do_not_merge(self):
        ...
        self.run_bzr_error(
            [
                "This command only works for merge mode packages.",
                "See /usr/share/doc/bzr-builddeb/user_manual"
                "/merge.html for more information.",
            ],
            "bd-do true",
        )
