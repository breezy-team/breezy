...
class LaunchpadAccountTests(TestCaseInTempDir):
    ...
    def test_no_registered_ssh_keys(self):
        ...
        self.assertEqualDiff(
            "The user test-user has not registered any "
    ...
    def test_get_lp_login_updates_authentication_conf(self):
        ...
        self.assertEqual("foo", auth.get_user("ssh", "git.launchpad.net"))
        self.assertEqual("foo", auth.get_user("ssh", "git.staging.launchpad.net"))

    def test_get_lp_login_leaves_existing_credentials(self):
        auth = config.AuthenticationConfig()
        auth.set_credentials("Foo", "git.launchpad.net", "foo", "ssh")
        auth.set_credentials("Bar", "git.staging.launchpad.net", "foo", "ssh")
        account._set_global_option("foo")
        account.get_lp_login()
        auth = config.AuthenticationConfig()
        credentials = auth.get_credentials("ssh", "git.launchpad.net")
    ...
    def test_get_lp_login_errors_on_mismatch(self):
        ...
        self.assertEqual(
            "breezy.conf and authentication.conf disagree about"
            " launchpad account name.  Please re-run launchpad-login.",
            str(e),
        )
...
class CheckAccountTests(TestCaseWithMemoryTransport):
    ...
    def test_check_lp_login_no_user(self):
        transport = self.get_transport()
        self.assertRaises(
            account.UnknownLaunchpadUsername,
    ...
    def test_check_lp_login_no_ssh_keys(self):
        ...
        self.assertRaises(
            account.NoRegisteredSSHKeys, account.check_lp_login, "test-user", transport
        )
