...
from ...tests import features, ssl_certs, TestCaseInTempDir, TestCaseWithMemoryTransport
from . import (
    _register_directory,
)
...
class LocalDirectoryURLTests(TestCaseInTempDir):
    ...
    def test_short_form_bzr(self):
        self.assertResolve("bzr+ssh://bazaar.launchpad.net/+branch/apt", "lp+bzr:apt")

    def test_two_part_form(self):
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.net/+branch/apt/2.2", "lp:apt/2.2"
        )
    ...
    def test_two_part_plus_subdir(self):
        ...
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.net/+branch/apt/2.2/BUGS", "lp:apt/2.2/BUGS"
        )

    def test_user_expansion(self):
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.net/~user/apt/foo", "lp:~/apt/foo"
        )
    ...
    def test_user_three_part(self):
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.net/~jameinel/apt/foo", "lp:~jameinel/apt/foo"
        )
    ...
    def test_user_ubuntu_three_part(self):
        self.assertResolve("bzr+ssh://fake-resolved", "lp:~jameinel/ubuntu/natty")
        self.assertResolve("bzr+ssh://fake-resolved", "lp:~jameinel/debian/sid")

    def test_user_ubuntu_four_part(self):
        self.assertResolve(
            "bzr+ssh://fake-resolved", "lp:~jameinel/ubuntu/natty/project"
        )
        self.assertResolve("bzr+ssh://fake-resolved", "lp:~jameinel/debian/sid/project")
    ...
    def test_handles_special_lp(self):
        ...
        self.assertResolve("bzr+ssh://bazaar.launchpad.net/+branch/apt", "lp:///apt")
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.net/+branch/apt", "lp://production/apt"
        )
        self.assertResolve(
            "bzr+ssh://bazaar.launchpad.test/+branch/apt", "lp://test/apt"
        )
        self.assertResolve(
            "bzr+ssh://bazaar.staging.launchpad.net/+branch/apt", "lp://staging/apt"
        )
        self.assertResolve(
            "bzr+ssh://bazaar.qastaging.launchpad.net/+branch/apt", "lp://qastaging/apt"
        )
        self.assertResolve(
            "bzr+ssh://bazaar.demo.launchpad.net/+branch/apt", "lp://demo/apt"
        )

    def test_debug_launchpad_uses_resolver(self):
        self.assertResolve("bzr+ssh://bazaar.launchpad.net/+branch/bzr", "lp:bzr")
...
class DirectoryUrlTests(TestCaseInTempDir):
    ...
    def test_short_form(self):
        ...
        factory = FakeResolveFactory(
            self, "apt", dict(urls=["http://bazaar.launchpad.net/~apt/apt/devel"])
        )
        directory = LaunchpadDirectory()
        self.assertEqual(
    ...
    def test_qastaging(self):
        ...
        factory = FakeResolveFactory(
            self,
            "apt",
            dict(urls=["http://bazaar.qastaging.launchpad.net/~apt/apt/devel"]),
        )
    ...
    def test_staging(self):
        ...
        factory = FakeResolveFactory(
            self,
            "apt",
            dict(urls=["http://bazaar.staging.launchpad.net/~apt/apt/devel"]),
        )
        url = "lp://staging/apt"
        directory = LaunchpadDirectory()
        self.assertEqual(
    ...
    def test_directory_no_matching_schemes(self):
        ...
        factory = FakeResolveFactory(
            self, "apt", dict(urls=["bad-scheme://bazaar.launchpad.net/~apt/apt/devel"])
        )
        directory = LaunchpadDirectory()
        self.assertRaises(urlutils.InvalidURL, directory._resolve, "lp:///apt", factory)
    ...
    def test_error_for_bad_url(self):
        ...
        self.assertRaises(
            urlutils.InvalidURL, directory._resolve, "lp://ratotehunoahu", factory
        )
    ...
    def test_tilde_fails_no_login(self):
        ...
        directory = LaunchpadDirectory()
        self.assertRaises(
            urlutils.InvalidURL, directory._resolve, "lp:~/apt/test", factory
        )
