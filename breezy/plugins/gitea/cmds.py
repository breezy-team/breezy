...
class cmd_gitea_login(Command):
    ...
    This command takes a Gitea instance URL (e.g. https://codeberg.org)
    as well as an optional access token. Access tokens can be created via the
    web UI.

    :Examples:

      Log into Codeberg (prompts for a token):

         brz gitea-login https://codeberg.org/

      Log into a Gitea instance, using a token created earlier:

         brz gitea-login https://gitea.example.com if4Theis6Eich7aef0zo
    """
    ...
    takes_options = [
        Option("name", help="Name for Gitea site in configuration.", type=str),
        Option("no-check", "Don't check that the token is valid."),
    ]
    ...
    def run(self, url, private_token=None, name=None, no_check=False):
        ...
        if private_token is None:
            note(
                "Please visit %s to obtain an access token.",
                urlutils.join(url, "user/settings/applications"),
            )
            private_token = ui.ui_factory.get_password("Access token")
        if not no_check:
