...
class BranchUpdater:
    def __init__(self, repo, branch, cache_mgr, heads_by_ref, last_ref, tags):
        ...
        self.tags = tags
        self._branch_format = best_format_for_objects_in_a_repository(repo)
    ...
    def _get_matching_branches(self):
        ...
        can_create_branches = (
            self.repo.is_shared() or self.repo.controldir._format.colocated_branches
        )
    ...
    def _update_branch(self, br, last_mark):
        ...
        if self.tags:
            graph = self.repo.get_graph()
            ancestry = [
                r for (r, ps) in graph.iter_ancestry([last_rev_id]) if ps is not None
            ]
        ...
        if changed:
            tagno = len(my_tags)
            note(
                "\t branch %s now has %d %s and %d %s",
                br.nick,
                revno,
                single_plural(revno, "revision", "revisions"),
                tagno,
                single_plural(tagno, "tag", "tags"),
            )
        return changed
