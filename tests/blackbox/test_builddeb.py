...
class TestBuilddeb(BuilddebTestCase):
    ...
    def test_builddeb_error_on_two_revisions(self):
        ...
        self.run_bzr_error(
            ['--revision takes exactly one revision specifier.'],
    ...
    def test_utf8_changelog(self):
        ...
        tree = self.make_unpacked_source()
        # The changelog is only used for commit message when it already exists
        # and is then changed, so need to clear it, commit, then set the
        # contents.
