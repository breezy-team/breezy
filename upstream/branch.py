    ...
    def version_as_revision(
        ...
        if revspec is not None:
            ...
            for revspec in guess_upstream_revspec(package, version):
                note(
                    gettext("No upstream upstream-revision format specified, trying %s")
                    % revspec
                )
                try:
                    return RevisionSpec.from_string(revspec).as_revision_id(
    ...
    def get_latest_version(self, package, current_version):
        if self.version_kind == "snapshot":
            ...
            if version_kind == "release":
                ...
                if version is None:
                    note(
                        gettext("No upstream releases found, falling back to snapshot.")
                    )
    ...
    def fetch_tarballs(
        ...
        with self.upstream_branch.lock_read():
            ...
            try:
                ...
                mutter(
                    "Exporting upstream branch revision %s to create the tarball", revid
                )
