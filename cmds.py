...
result_opt = Option(
    "result-dir",
    help="Directory in which to place the resulting package files.",
    type=str,
    argname="DIRECTORY",
)
builder_opt = Option(
    "builder", help="Command to build the package.", type=str, argname="BUILDER"
)
merge_opt = Option(
    "merge", help="Merge the debian part of the source in to the upstream tarball."
)
split_opt = Option(
    "split", help="Automatically create an .orig.tar.gz from a full source branch."
)
...
native_opt = Option("native", help="Build a native package.")
...
apt_repository_opts = [
    Option("apt-repository", help="Apt repository to attempt to fetch from", type=str),
    Option(
        "apt-repository-key", help="Apt repository key to use for validation", type=str
    ),
]
...
def _get_changelog_info(
    ...
    try:
        ...
        if distribution is None:
            ...
            note(
                gettext("No distribution specified, and no changelog, assuming '%s'"),
                distribution,
            )
    else:
        ...
        if distribution is None:
            ...
            if distribution is not None:
                ...
                note(
                    gettext(
                        "No distribution specified, and no previous "
    ...
    if distribution_name is None:
        raise BzrCommandError(gettext("Unknown target distribution: %s") % distribution)
    return (
        current_version,
        package,
        distribution,
...
def _get_upstream_sources(
    ...
    upstream_branch_source = _get_upstream_branch_source(
        ...
        config,
        upstream_version,
        other_repository=packaging_branch.repository,
...
def _get_distiller(
    ...
    if tree.has_filename(os.path.join(subpath, "debian/debcargo.toml")):
        return DebcargoDistiller(
            tree, subpath, top_level=top_level, use_existing=use_existing
        )
...
class cmd_builddeb(Command):
    ...
    export_only_opt = Option(
        "export-only", help="Export only, don't build.", short_name="e"
    )
    use_existing_opt = Option("use-existing", help="Use an existing build directory.")
    quick_opt = Option(
        "quick",
        help="Quickly build the package, uses "
        'quick-builder, which defaults to "fakeroot '
        'debian/rules binary".',
    )
    reuse_opt = Option(
        "reuse",
        help="Try to avoid exporting too much on each "
    ...
    guess_upstream_branch_url_opt = Option(
        ...
        help=("Guess upstream branch URL if unknown (requires upstream-ontologist)"),
    )
    ...
    def _get_build_tree(self, revision, tree, branch):
        if revision is None and tree is not None:
            ...
            if revision is None:
                ...
                raise BzrCommandError(
                    gettext(
                        "bzr builddeb --revision takes exactly one revision specifier."
                    )
                )
    ...
    def _get_dirs(self, config, location, is_local, result_dir, build_dir, orig_dir):
            ...
            if supplied is not None:
                if is_local:
                    supplied = os.path.join(
                        urlutils.local_path_from_url(location), supplied
                    )
        ...
        build_dir = _get_dir(
            build_dir,
            config.build_dir or default_build_dir,
            config.user_build_dir or "build-area",
        )
        orig_dir = _get_dir(
            orig_dir,
            config.orig_dir or default_orig_dir,
            config.user_orig_dir or "build-area",
        )
        return result_dir, build_dir, orig_dir

    def _branch_and_build_options(self, branch_or_build_options_list, source=False):
        branch = None
        build_options = []
        source_opt = False
    ...
    def run(
        self,
        branch_or_build_options_list=None,
        verbose=False,
        export_only=False,
        dont_purge=False,
        use_existing=False,
        ...
        tree, branch, is_local, location, subpath = self._get_tree_and_branch(location)
        ...
        with tree.lock_read():
            ...
            try:
                ...
                raise BzrCommandError(
                    gettext("Unable to parse changelog: %s") % e
                ) from e
            ...
            build_cmd = self._get_build_command(config, builder, quick, build_options)
            result_dir, build_dir, orig_dir = self._get_dirs(
                config, location or ".", is_local, result_dir, build_dir, orig_dir
            )
            ...
            builder = DebBuild(
                distiller, build_source_dir, build_cmd, use_existing=use_existing
            )
            builder.prepare()
            ...
            if not export_only:
                ...
                if is_local:
                    ...
                    target_dir = os.path.join(
                        urlutils.local_path_from_url(location), target_dir
                    )
                else:
                    target_dir = "."
...
class cmd_get_orig_source(Command):
    ...
    def run(
        ...
        upstream_sources = [get_pristine_tar_source(tree, tree.branch), AptSource(apt)]
        try:
        ...
        upstream_provider = UpstreamProvider(
            changelog.package, str(version), orig_dir, upstream_sources
        )
...
class cmd_merge_upstream(Command):
    ...
    package_opt = Option("package", help="The name of the source package.", type=str)
    version_opt = Option(
        "version",
        help='The upstream version number of this release, for example "0.2".',
        type=str,
    )
    distribution_opt = Option(
        "distribution",
        help="The distribution that this release is targetted at.",
        type=str,
    )
    directory_opt = Option(
        "directory", help="Working tree into which to merge.", short_name="d", type=str
    )
    last_version_opt = Option(
        "last-version",
        help="The full version of the last time upstream was merged.",
        type=str,
    )
    force_opt = Option(
        "force", help=("Force a merge even if the upstream branch has not changed.")
    )
    snapshot_opt = Option(
        "snapshot",
        help="Merge a snapshot from the "
        "upstream branch rather than a new upstream release.",
    )

    release_opt = Option("release", help="Merge a release from the upstream branch.")

    force_pristine_tar_opt = Option(
        "force-pristine-tar",
        help=("Force the use of pristine-tar, even if no pristine-tar branch exists"),
    )

    dist_command_opt = Option(
        "dist-command",
        type=str,
        help="Command to run for creating an upstream tarball from a VCS snapshot.",
    )
    ...
    def run(
        ...
        with tree.lock_write():
            ...
            (
                ...
                changelog,
                top_level,
            ) = _get_changelog_info(tree, subpath, last_version, package, distribution)
            if package is None:
                raise BzrCommandError(
            ...
            if snapshot is None and release is None:
                ...
                raise BzrCommandError("--release and --snapshot are incompatible")
            elif snapshot:
            ...
            if build_type == BUILD_TYPE_NATIVE:
                raise BzrCommandError(
                    gettext("Merge upstream in native mode is not supported.")
                )
            ...
            if upstream_branch is not None:
                ...
                note(
                    gettext("Using upstream branch %s (from configuration)"),
            ...
            if location is not None:
                ...
                if snapshot:
                    if upstream_branch_source is None:
                        raise BzrCommandError(
                            gettext("--snapshot requires an upstream branch source")
                        )
            ...
            if revision is not None:
                ...
                if len(revision) > 1:
                    raise BzrCommandError(
                        gettext("merge-upstream takes only a single --revision")
                    )
                upstream_revspec = revision[0]
                upstream_revisions = {
                    None: (upstream_revspec.as_revision_id(upstream_branch), subpath)
                }
            else:
            ...
            if version is None:
                if upstream_branch_source is not None:
                    ...
                    raise BzrCommandError(
                        gettext("You must specify the version number using --version.")
                    )
            ...
            if need_upstream_tarball:
                ...
                try:
                    ...
                    raise BzrCommandError(str(e)) from e
            if current_version is not None and Version(current_version) >= Version(
                version
            ):
                raise BzrCommandError(
                    gettext("Upstream version %s has already been merged.") % version
                )
            changelog_add_new_version(
        ...
        if not need_upstream_tarball:
            note(
                gettext(
                    "An entry for the new upstream version has been "
            ...
            if conflicts:
                note(
                    gettext(
                        "You should now resolve the conflicts, review "
                        "the changes, and then commit."
                    )
                )
            else:
                note(gettext("You should now review the changes and then commit."))
...
class cmd_import_dsc(Command):
    ...
    filename_opt = Option(
        "file",
        help="File containing URIs of source packages to import.",
        type=str,
        short_name="F",
    )
    ...
    def run(self, files_list, file=None):
        ...
        try:
            ...
            raise BzrCommandError(
                gettext("There is no tree to import the packages in to")
            ) from e
        with tree.lock_write():
            _check_uncommitted(tree, subpath)
...
class cmd_import_upstream(Command):
    ...
    def run(
        ...
        if upstream_branch is None:
            ...
            upstream, upstream_subpath = Branch.open_containing(upstream_branch)
        self.add_cleanup(branch.lock_write().unlock)
        tempdir = self.enter_context(
        ...
        if base_revisions:
            ...
            db.extract_upstream_tree(
                {comp: (revid, subpath) for comp, revid in base_revisions.items()},
                tempdir,
            )
            parents = {}
        ...
        for (
            ...
            if component is None:
                self.outf.write(
                    gettext("Imported %(location)s as tag:%(tag)s.\n")
                    % {"location": location, "tag": tag_name}
                )
            else:
                self.outf.write(
                    gettext("Imported %(location)s (%(component)s) as tag:%(tag)s.\n")
                    % {"location": location, "component": component, "tag": tag_name}
                )
...
class cmd_builddeb_do(Command):
    ...
    def run(
        ...
        (changelog, top_level) = find_changelog(t, subpath, merge=False, max_blocks=2)
        ...
        build_source_dir = os.path.join(
            build_dir, changelog.package + "-" + changelog.version.upstream_version
        )
        ...
        if give_instruction:
            note(
                gettext(
                    "If you want to cancel your changes then exit "
        ...
        try:
            ...
            raise BzrCommandError(
                gettext("Not updating the working tree as the command failed.")
            ) from e
        finally:
            if not no_preparation:
                try:
                    ...
                    raise BzrCommandError(
                        gettext("After build processing failed. Aborting.")
                    ) from e
        ...
        note(
            gettext(
                "If any files were added or removed you should run "
...
class cmd_dep3_patch(Command):
    ...
    no_upstream_check_opt = Option(
        "no-upstream-check", help="Don't check whether patch has been merged upstream."
    )
...
def _build_helper(
    ...
    contains_upstream_source = tree_contains_upstream_source(local_tree, subpath)
