...
#[cfg(windows)]
fn color_name_is_valid(name: &str) -> bool {
    matches!(
        name,
        "darkblack"
            | "darkred"
...
fn colorstring<'a>(
    ...
    if let Some(name) = fgcolor {
        if !color_name_is_valid(name) {
    ...
    Ok(PyBytes::new(
        py,
        &breezy_osutils::terminal::colorstring(text, None, None),
    ))
}
