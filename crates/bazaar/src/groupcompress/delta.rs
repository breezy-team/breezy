...
///
/// Ported to Rust by Jelmer Vernooĳ <jelmer@jelmer.uk>
///
...
struct SourceInfo {
    buf: Vec<u8>,
    agg_offset: usize, // Start of source data as part of the aggregate source
}
...
pub enum DeltaError {
    ...
    SourceBad,   // A source_info had invalid or corrupt content
    BufferEmpty, // A buffer pointer and size
    SizeTooBig,  // Delta data is larger than the max requested
}

const DELTA_SIZE_MIN: u64 = 3;

fn get_delta_hdr_size(datap: &mut &[u8], top: &[u8]) -> u64 {
    let mut data = *datap;
    let mut cmd: u8;
    let mut size: u64 = 0;
    let mut i = 0;
    ...
    loop {
        cmd = data[0];
        data = &data[1..];
        size |= u64::from(cmd & !0x80) << i;
        i += 7;

        if (cmd & 0x80) == 0 || data >= top {
            break;
        }
    }
...
fn get_text(buff: &mut [u8; 128], ptr: &[u8]) {
    let start = &ptr[ptr.len().saturating_sub(RABIN_WINDOW + 1)..];
    let mut cmd = start[0];
    if cmd < 0x80 {
        // This is likely to be an insert instruction
        if cmd < RABIN_WINDOW as u8 {
            cmd = RABIN_WINDOW as u8;
        }
    } else {
        // This was either a copy [should never be] or it
        // was a longer insert so the insert start happened at 16 more
        // bytes back.
        cmd = RABIN_WINDOW as u8 + 1;
    }
    if cmd > 60 {
    ...
    cmd += 5;
    buff[..cmd as usize].copy_from_slice(&start[..cmd as usize]);
    buff[cmd as usize] = 0;
    for i in 0..cmd as usize {
        match buff[i] {
            b'\n' => buff[i] = b'N',
            b'\t' => buff[i] = b'T',
            _ => {}
...
fn rabin_hash(data: &[u8]) -> u32 {
    ...
    for c in data.iter().take(RABIN_WINDOW) {
        val = ((val << 8) | *c as u32) ^ T[(val >> RABIN_SHIFT) as usize];
    }
    val
}
