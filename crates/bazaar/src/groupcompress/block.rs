use crate::groupcompress::{apply_delta_to_source, decode_base128_int, decode_copy_instruction};
use std::borrow::Cow;
use std::io::BufRead;
...
impl GroupCompressBlock {
    ...
    pub fn ensure_content(&mut self, num_bytes: Option<usize>) {
        ...
        let num_read = self
            .z_content_decompressor
            .as_mut()
            .unwrap()
            .read(&mut buf)
            .unwrap();
        assert_eq!(num_read, num_bytes);
        self.content.as_mut().unwrap().extend(buf);

        assert!(
            self.content.as_ref().unwrap().len() >= num_bytes,
            "{} bytes wanted, only {} available",
            num_bytes,
    ...
    pub fn extract(&mut self, start: usize, end: usize) -> Result<Vec<Vec<u8>>, Error> {
        ...
        let c = self.content.as_ref().unwrap()[start];
        match c {
            b'f' | b'd' => {}
            c => {
        ...
        let (content_len, len_len) =
            decode_base128_int(&self.content.as_ref().unwrap()[start + 1..start + 6]);
        let content_start = start + 1 + len_len;
        if end != content_start + content_len as usize {
            return Err(Error::InvalidData(format!(
                "end != len according to field header: {} != {}",
                end,
                content_start + content_len as usize
            )));
        }
        match c {
            b'f' => {
                // Fulltext
                Ok(vec![
                    self.content.as_ref().unwrap()[content_start..end].to_vec()
                ])
            }
            b'd' => {
                // Must be type delta as checked above
                Ok(vec![apply_delta_to_source(
    ...
    pub fn dump(&mut self, include_text: Option<bool>) -> Result<Vec<DumpInfo>, Error> {
        ...
        let mut result = vec![];
        let mut pos = 0;
        while pos < self.content_length.unwrap() {
            let kind = self.content.as_ref().unwrap()[pos];
            pos += 1;
            match kind {
                b'f' | b'd' => {}
                _ => {
            ...
            let (content_len, len_len) =
                decode_base128_int(&self.content.as_ref().unwrap()[pos..pos + 5]);
            pos += len_len;
            if content_len as usize + pos > self.content_length.unwrap() {
                return Err(Error::InvalidData(format!(
                    "invalid content_len {} for record @ pos {}",
                    content_len,
                    pos - len_len - 1
                )));
            }
            match kind {
                b'f' => {
                    // Fulltext
                    if include_text {
                        let text = &self.content.as_ref().unwrap()[pos..pos + content_len as usize];
                ...
                b'd' => {
                    // Delta
                    let delta_content =
                        &self.content.as_ref().unwrap()[pos..pos + content_len as usize];
                    let mut delta_info = vec![];
                    // The first entry in a delta is the decompressed length
                    let (decomp_len, mut delta_pos) = decode_base128_int(delta_content);
                    let mut measured_len = 0;
                    while delta_pos < content_len as usize {
                        let c = delta_content[delta_pos];
                        delta_pos += 1;
                        if c & 0x80 > 0 {
                            // Copy
                            let (offset, length, _delta_pos) =
                                decode_copy_instruction(delta_content, c, delta_pos).unwrap();
                            if include_text {
                                let text = &self.content.as_ref().unwrap()[offset..offset + length];
                                ...
                                delta_info.push(DeltaInfo::Copy(offset, length, None));
                            }
                            measured_len += length;
                        } else {
                            // Insert
                            let txt = if include_text {
                                &delta_content[delta_pos..delta_pos + c as usize]
                            } else {
                                &b""[..]
                            };
                            delta_info.push(DeltaInfo::Insert(c as usize, txt.to_vec()));
                            measured_len += c as usize;
                            delta_pos += c as usize;
                        }
                    }
                    if delta_pos != content_len as usize {
                        return Err(Error::InvalidData(format!(
                            "Delta consumed a bad number of bytes: {} != {}",
                            delta_pos, content_len
                        )));
                    }
