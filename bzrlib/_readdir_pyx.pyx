...
cdef _read_dir(path):
    ...
    if path != "" and path != '.':
        ...
        if orig_dir_fd == -1:
            raise OSError(errno, strerror(errno), "opening original directory")
        if -1 == chdir(path):
            raise OSError(errno, strerror(errno), "chdir: " + path)
    else:
        orig_dir_fd = -1
    ...
    try:
        the_dir = opendir(".")
        if NULL == the_dir:
            raise OSError(errno, strerror(errno), "opendir: " + path)
        try:
            result = []
            entry = &sentinel
            while entry != NULL:
                ...
                if entry == NULL:
                    if errno == ENOTDIR or errno == 0:
                        ...
                        raise OSError(errno, strerror(errno), "readdir: " + path)
                name = entry.d_name
                if not (name[0] == c"." and (
                    (name[1] == 0) or 
            ...
            if -1 == closedir(the_dir):
                raise OSError(errno, strerror(errno), "closedir: " + path)
    finally:
        if -1 != orig_dir_fd:
            failed = False
            ...
            if -1 == close(orig_dir_fd) or failed:
                raise OSError(errno, strerror(errno), "return to orig_dir_fd")
