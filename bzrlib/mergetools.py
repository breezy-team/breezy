...
class MergeTool(object):
    ...
    def _expand_commandline(self, filename):
        ...
        tmp_file = None
        commandline = commandline.replace('%b', filename + '.BASE')
        commandline = commandline.replace('%t', filename + '.THIS')
        commandline = commandline.replace('%o', filename + '.OTHER')
        commandline = commandline.replace('%r', filename)
        if '%T' in commandline:
            tmp_file = tempfile.mktemp("_bzr_mergetools_%s.THIS" % os.path.basename(filename))
            shutil.copy(filename + ".THIS", tmp_file)
            commandline = commandline.replace('%T', tmp_file)
