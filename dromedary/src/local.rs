...
impl LocalTransport {
    ...
    fn _abspath(&self, relative_reference: &str) -> Result<PathBuf> {
        if relative_reference == "." || relative_reference.is_empty() {
            ...
            let extra = crate::urlutils::unescape(relative_reference)?;
...
impl Transport for LocalTransport {
    ...
    fn local_abspath(&self, relpath: &UrlFragment) -> Result<PathBuf> {
        ...
        crate::urlutils::local_path_from_url(absurl.as_str()).map_err(Error::from)
    }
    ...
    fn abspath(&self, relpath: &UrlFragment) -> Result<Url> {
        ...
        let url_str = crate::urlutils::local_path_to_url(path.as_path())
            .map_err(|e| map_io_err_to_transport_err(e, Some(relpath)))?;
        Ok(Url::parse(&url_str).unwrap())
    }

    fn relpath(&self, abspath: &Url) -> Result<String> {
        let relpath = crate::urlutils::file_relpath(self.base.as_str(), abspath.as_str())
            .map_err(Error::from)?;
        Ok(relpath)
    }
    ...
    fn symlink(&self, source: &UrlFragment, link_name: &UrlFragment) -> Result<()> {
        ...
        let abs_link_dirpath = crate::urlutils::dirname(abs_to.as_str(), true);
        let source_rel = crate::urlutils::file_relpath(
