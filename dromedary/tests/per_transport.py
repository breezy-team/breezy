...
class TransportTests(TestTransportImplementation):
    ...
    def test_put_bytes(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.put_bytes, "a", b"some text for a\n"
            )
            return
    ...
    def test_put_bytes_non_atomic(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.put_bytes_non_atomic, "a", b"some text for a\n"
            )
            return
        ...
        self.assertRaises(
            NoSuchFile, t.put_bytes_non_atomic, "no/such/path", b"contents\n"
        )
    ...
    def test_put_file(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.put_file, "a", BytesIO(b"some text for a\n")
            )
            return
        ...
        self.check_transport_contents(b"new\ncontents for\na\n", t, "a")
        self.assertRaises(
            NoSuchFile, t.put_file, "path/doesnt/exist/c", BytesIO(b"contents")
        )
    ...
    def test_put_file_non_atomic(self):
        ...
        self.assertRaises(
            NoSuchFile, t.put_file_non_atomic, "no/such/path", BytesIO(b"contents\n")
        )
        # Now test the create_parent flag
        self.assertRaises(
            NoSuchFile, t.put_file_non_atomic, "dir/a", BytesIO(b"contents\n")
        )
        self.assertFalse(t.has("dir/a"))
        t.put_file_non_atomic(
            "dir/a", BytesIO(b"contents for dir/a\n"), create_parent_dir=True
        )
        self.check_transport_contents(b"contents for dir/a\n", t, "dir/a")
    ...
    def test_put_file_non_atomic_permissions(self):
        ...
        sio = BytesIO()
        t.put_file_non_atomic(
            "dir700/mode664", sio, mode=0o664, dir_mode=0o700, create_parent_dir=True
        )
        self.assertTransportMode(t, "dir700", 0o700)
        t.put_file_non_atomic(
            "dir770/mode664", sio, mode=0o664, dir_mode=0o770, create_parent_dir=True
        )
        self.assertTransportMode(t, "dir770", 0o770)
        t.put_file_non_atomic(
            "dir777/mode664", sio, mode=0o664, dir_mode=0o777, create_parent_dir=True
        )
        self.assertTransportMode(t, "dir777", 0o777)
    ...
    def test_mkdir(self):
        ...
        self.assertEqual(
            [t.has(n) for n in ["dir_a", "dir_b", "dir_q", "dir_b"]],
    ...
    def test_opening_a_file_stream_can_set_mode(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                (TransportNotPossible, NotImplementedError), t.open_write_stream, "foo"
            )
            return
        if not t._can_roundtrip_unix_modebits():
            # Can't roundtrip, so no need to run this test
        ...
        def simple_copy_files(transport_from, transport_to):
            ...
            for f in files:
                self.check_transport_contents(
                    transport_to.get_bytes(f), transport_from, f
                )
    ...
    def test_append_file(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.append_file, "a", "add\nsome\nmore\ncontents\n"
            )
            return
        ...
        self.assertEqual(
            20, t.append_file("a", BytesIO(b"add\nsome\nmore\ncontents\n"))
        )
        ...
        self.assertRaises(
            NoSuchFile, t.append_file, "missing/path", BytesIO(b"content")
        )

        # And we can create new files, too
        self.assertEqual(
            0, t.append_file("c", BytesIO(b"some text\nfor a missing file\n"))
        )
    ...
    def test_append_file_mode(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.append_file, "f", BytesIO(b"f"), mode=None
            )
            return
    ...
    def test_append_bytes_mode(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                TransportNotPossible, t.append_bytes, "f", b"f", mode=None
            )
            return
    ...
    def test_rename_dir_succeeds(self):
        ...
        if t.is_readonly():
            self.assertRaises(
                (TransportNotPossible, NotImplementedError), t.rename, "foo", "bar"
            )
            return
        t.mkdir("adir")
    ...
    def test_clone_to_root(self):
        ...
        self.assertTrue(
            len(new_transport.base) < len(root_transport.base)
    ...
    def test_clone_from_root(self):
        ...
        self.assertEqual(
            root_transport.base + ".bzr/", root_transport.clone(".bzr").base
        )
    ...
    def test_readv_with_adjust_for_latency(self):
        ...
        result = list(
            transport.readv(
                "a", ((0, 30),), adjust_for_latency=True, upper_limit=content_size
            )
        )
        ...
        result = list(
            transport.readv(
                "a", ((204700, 100),), adjust_for_latency=True, upper_limit=content_size
            )
        )
    ...
    def test_readv_short_read(self):
        ...
        self.assertListRaises(
            (errors.ShortReadvError, errors.InvalidRange),
            transport.readv,
            "a",
            [(12, 2)],
        )
