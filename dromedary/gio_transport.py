...
try:
    ...
    raise DependencyNotPresent("glib", e) from e
try:
    import gio
except ModuleNotFoundError as e:
...
class GioTransport(ConnectedTransport):
    ...
    def has(self, relpath):
        ...
        try:
            debug_logger.debug("GIO has check: %s", relpath)
    ...
    def get(self, relpath, retries=0):
        ...
        """
        try:
            debug_logger.debug("GIO get: %s", relpath)
            f = self._get_GIO(relpath)
            fin = f.read()
            buf = fin.read()
            fin.close()
    ...
    def put_file(self, relpath, fp, mode=None):
        ...
        """
        debug_logger.debug("GIO put_file %s", relpath)
    ...
    def mkdir(self, relpath, mode=None):
        ...
        try:
            debug_logger.debug("GIO mkdir: %s", relpath)
            f = self._get_GIO(relpath)
            f.make_directory()
            self._setmode(relpath, mode)
        except gio.Error as e:
    ...
    def open_write_stream(self, relpath, mode=None):
        ...
        debug_logger.debug("GIO open_write_stream %s", relpath)
        if mode is not None:
            self._setmode(relpath, mode)
    ...
    def recommended_page_size(self):
        ...
        """
        debug_logger.debug("GIO recommended_page")
        return 64 * 1024

    def rmdir(self, relpath):
        """Delete the directory at rel_path."""
        try:
            debug_logger.debug("GIO rmdir %s", relpath)
            ...
            logger.debug("failed to rmdir %s: %s", relpath, e)
    ...
    def append_file(self, relpath, file, mode=None):
        ...
        # Work around this.
        debug_logger.debug("GIO append_file: %s", relpath)
        tmppath = "%s.tmp.%.9f.%d.%d" % (
            relpath,
            time.time(),
            os.getpid(),
    ...
    def _setmode(self, relpath, mode):
        ...
        """
        debug_logger.debug("GIO _setmode %s", relpath)
        if mode:
            try:
                f = self._get_GIO(relpath)
    ...
    def rename(self, rel_from, rel_to):
        ...
        try:
            debug_logger.debug("GIO move (rename): %s => %s", rel_from, rel_to)
    ...
    def move(self, rel_from, rel_to):
        ...
        try:
            debug_logger.debug("GIO move: %s => %s", rel_from, rel_to)
            f = self._get_GIO(rel_from)
            t = self._get_GIO(rel_to)
    ...
    def delete(self, relpath):
        ...
        try:
            debug_logger.debug("GIO delete: %s", relpath)
            f = self._get_GIO(relpath)
            f.delete()
        except gio.Error as e:
            self._translate_gio_error(e, relpath)
    ...
    def external_url(self):
        ...
        debug_logger.debug("GIO external_url %s", self.base)
        # GIO external url
        return self.base

    def listable(self):
        """See Transport.listable."""
        debug_logger.debug("GIO listable")
        return True

    def list_dir(self, relpath):
        """See Transport.list_dir."""
        debug_logger.debug("GIO list_dir")
        try:
            entries = []
            f = self._get_GIO(relpath)
    ...
    def iter_files_recursive(self):
        ...
        """
        debug_logger.debug("GIO iter_files_recursive")
    ...
    def stat(self, relpath):
        ...
        try:
            debug_logger.debug("GIO stat: %s", relpath)
            f = self._get_GIO(relpath)
    ...
    def lock_read(self, relpath):
        ...
        """
        debug_logger.debug("GIO lock_read %s", relpath)
    ...
    def lock_write(self, relpath):
        ...
        """
        debug_logger.debug("GIO lock_write %s", relpath)
        return self.lock_read(relpath)

    def _translate_gio_error(self, err, path, extra=None):
        debug_logger.debug("GIO Error: %s %s", err, path)
        if extra is None:
            extra = str(err)
        if err.code == gio.ERROR_NOT_FOUND:
            ...
            raise PermissionDenied(path, extra=extra)
        elif err.code == gio.ERROR_HOST_NOT_FOUND:
            raise errors.PathError(path, extra=extra)
        elif err.code == gio.ERROR_IS_DIRECTORY:
            raise errors.PathError(path, extra=extra)
        else:
            logger.debug("unable to understand error for path: %s: %s", path, err)
