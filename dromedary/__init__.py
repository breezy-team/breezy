...
class FileStream:
    ...
    def fdatasync(self):
        ...
        :raises errors.TransportNotPossible: If this transport has no way to
            flush to disk.
        """
...
class Transport:
    ...
    def _translate_error(self, e, path, raise_generic=True):
        ...
        if getattr(e, "errno", None) is not None:
            if e.errno in (errno.ENOENT, errno.ENOTDIR):
                ...
                logger.debug("EINVAL returned on path %s: %r", path, e)
                raise errors.NoSuchFile(path, extra=e)
        ...
        if isinstance(e, (NotADirectoryError, FileNotFoundError)):
            raise errors.NoSuchFile(path, extra=e)
        if raise_generic:
            raise errors.TransportError(msg="Transport operation failed", orig_error=e)
    ...
    def get(self, relpath):
        ...
        This may fail in a number of ways:
         - HTTP servers may return content for a directory. (unexpected
           content failure)
         - FTP servers may indicate errors.NoSuchFile for a directory.
         - SFTP servers may give a file handle for a directory that will
           fail on read().

        For correct use of the interface, be sure to catch PathError
    ...
    def rename(self, rel_from, rel_to):
        ...
        not automatically remove it.  It should raise errors.DirectoryNotEmpty, or
        some other errors.PathError if the case can't be specifically detected.
    ...
    def lock_read(self, relpath):
        ...
        Transports may raise errors.TransportNotPossible if OS-level locks cannot be
    ...
    def _redirected_to(self, source, target):
        ...
        :returns: A transport
        :raise errors.UnusableRedirect: when redirection can not be provided
        """
...
def _try_transport_factories(base, factory_list):
    ...
    for factory in factory_list:
        try:
            ...
            logger.debug(
                "failed to instantiate transport %r for %r: %r", factory, base, e
            )
            last_err = e
            continue
...
def do_catching_redirections(
    ...
    Args:
      ...
      redirected: A callable receiving the redirected transport and the
                  errors.RedirectRequested exception.
...
register_lazy_transport("file://", "dromedary.local", "LocalTransport")
...
register_lazy_transport("sftp://", "dromedary.sftp", "SFTPTransport")
...
register_lazy_transport("http+urllib://", "dromedary.http.urllib", "HttpTransport")
register_transport_proto(
    "https+urllib://",
    #                help="Read-only access of branches exported on the web using SSL."
    register_netloc=True,
)
register_lazy_transport("https+urllib://", "dromedary.http.urllib", "HttpTransport")
...
register_lazy_transport("memory://", "dromedary.memory", "MemoryTransport")

register_transport_proto(
    "readonly+",
    #              help="This modifier converts any transport to be readonly."
)
register_lazy_transport("readonly+", "dromedary.readonly", "ReadonlyTransportDecorator")

register_transport_proto("fakenfs+")
register_lazy_transport("fakenfs+", "dromedary.fakenfs", "FakeNFSTransportDecorator")

register_transport_proto("log+")
register_lazy_transport("log+", "dromedary.log", "TransportLogDecorator")

register_transport_proto("trace+")
register_lazy_transport("trace+", "dromedary.trace", "TransportTraceDecorator")

register_transport_proto("unlistable+")
register_lazy_transport(
    "unlistable+", "dromedary.unlistable", "UnlistableTransportDecorator"
)

register_transport_proto("brokenrename+")
register_lazy_transport(
    "brokenrename+", "dromedary.brokenrename", "BrokenRenameTransportDecorator"
)
