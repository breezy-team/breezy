...
from brzlib.transport import chroot, get_transport
from brzlib.urlutils import local_path_to_url
...
def make_app(root, prefix, path_var='REQUEST_URI', readonly=True,
    ...
    if enable_logging:
        import brzlib.trace
        brzlib.trace.enable_default_logging()
...
class RelpathSetter(object):
    """WSGI middleware to set 'brzlib.relpath' in the environ.
    ...
    'brzlib.relpath' variable to "repo/branch".
    """
    ...
    def __init__(self, app, prefix='', path_var='REQUEST_URI'):
        ...
        :param path_var: the variable in the WSGI environ to calculate the
            'brzlib.relpath' variable from.
        :param prefix: a prefix to strip from the variable specified in
            path_var before setting 'brzlib.relpath'.
        """
        self.app = app
        self.prefix = prefix
    ...
    def __call__(self, environ, start_response):
        ...
        if not (path.startswith(self.prefix) and path.endswith(suffix)):
            ...
            return []
        environ['brzlib.relpath'] = path[len(self.prefix):-len(suffix)]
...
class SmartWSGIApp(object):
    ...
    def __init__(self, backing_transport, root_client_path='/'):
        ...
        # brzlib that will invoke 'get_transport' on urls rather than cloning
    ...
    def __call__(self, environ, start_response):
        ...
        relpath = environ['brzlib.relpath']
