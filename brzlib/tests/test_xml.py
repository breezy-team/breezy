...
class TestSerializer(TestCase):
    ...
    def test_unpack_revision_5(self):
        ...
        rev = brzlib.xml5.serializer_v5.read_revision(inp)
        eq = self.assertEqual
        eq(rev.committer,
    ...
    def test_unpack_inventory_5(self):
        ...
        inv = brzlib.xml5.serializer_v5.read_inventory(inp)
        eq = self.assertEqual
        eq(len(inv), 4)
    ...
    def test_unpack_inventory_5a(self):
        inv = brzlib.xml5.serializer_v5.read_inventory_from_string(
    ...
    def test_repack_inventory_5(self):
        ...
        inv = brzlib.xml5.serializer_v5.read_inventory(inp)
        outp = StringIO()
        brzlib.xml5.serializer_v5.write_inventory(inv, outp)
        self.assertEqualDiff(_expected_inv_v5, outp.getvalue())
        inv2 = brzlib.xml5.serializer_v5.read_inventory(StringIO(outp.getvalue()))
    ...
    def assertRoundTrips(self, xml_string):
        ...
        lines = brzlib.xml5.serializer_v5.write_inventory_to_lines(inv)
        outp.seek(0)
    ...
    def check_repack_revision(self, txt):
        ...
        rev = brzlib.xml5.serializer_v5.read_revision(inp)
        outp = StringIO()
        brzlib.xml5.serializer_v5.write_revision(rev, outp)
        outfile_contents = outp.getvalue()
        rev2 = brzlib.xml5.serializer_v5.read_revision(StringIO(outfile_contents))
    ...
    def test_pack_revision_5(self):
        ...
        rev = brzlib.xml5.serializer_v5.read_revision_from_string(_revision_v5)
        outp = StringIO()
        brzlib.xml5.serializer_v5.write_revision(rev, outp)
        outfile_contents = outp.getvalue()
        self.assertEqual(outfile_contents[-1], '\n')
        self.assertEqualDiff(outfile_contents, brzlib.xml5.serializer_v5.write_revision_to_string(rev))
    ...
    def test_empty_property_value(self):
        ...
        s_v5 = brzlib.xml5.serializer_v5
    ...
    def test_roundtrip_inventory_v7(self):
        ...
        self.assertEqual(brzlib.osutils.split_lines(txt), lines)
    ...
    def test_wrong_format_v7(self):
        ...
        s_v6 = brzlib.xml6.serializer_v6
    ...
    def test_revision_text_v6(self):
        ...
        rev = brzlib.xml6.serializer_v6.read_revision_from_string(
            _expected_rev_v5)
        serialized = brzlib.xml6.serializer_v6.write_revision_to_string(rev)
        self.assertEqualDiff(serialized, _expected_rev_v5)

    def test_revision_text_v7(self):
        """Pack revision to XML v7"""
        rev = brzlib.xml7.serializer_v7.read_revision_from_string(
            _expected_rev_v5)
        serialized = brzlib.xml7.serializer_v7.write_revision_to_string(rev)
        self.assertEqualDiff(serialized, _expected_rev_v5)

    def test_revision_text_v8(self):
        """Pack revision to XML v8"""
        rev = brzlib.xml8.serializer_v8.read_revision_from_string(
            _expected_rev_v8)
        serialized = brzlib.xml8.serializer_v8.write_revision_to_string(rev)
...
class TestEncodeAndEscape(TestCase):
    ...
    def setUp(self):
        ...
        brzlib.xml_serializer._clear_cache()
        self.addCleanup(brzlib.xml_serializer._clear_cache)

    def test_simple_ascii(self):
        # _encode_and_escape always appends a final ", because these parameters
        # are being used in xml attributes, and by returning it now, we have to
        # do fewer string operations later.
        val = brzlib.xml_serializer.encode_and_escape('foo bar')
        self.assertEqual('foo bar"', val)
        # The second time should be cached
        val2 = brzlib.xml_serializer.encode_and_escape('foo bar')
        self.assertIs(val2, val)

    def test_ascii_with_xml(self):
        self.assertEqual('&amp;&apos;&quot;&lt;&gt;"',
                         brzlib.xml_serializer.encode_and_escape('&\'"<>'))

    def test_utf8_with_xml(self):
        # u'\xb5\xe5&\u062c'
        utf8_str = '\xc2\xb5\xc3\xa5&\xd8\xac'
        self.assertEqual('&#181;&#229;&amp;&#1580;"',
                         brzlib.xml_serializer.encode_and_escape(utf8_str))

    def test_unicode(self):
        uni_str = u'\xb5\xe5&\u062c'
        self.assertEqual('&#181;&#229;&amp;&#1580;"',
                         brzlib.xml_serializer.encode_and_escape(uni_str))
...
class TestMisc(TestCase):
    ...
    def test_unescape_xml(self):
        ...
        self.assertRaises(KeyError, brzlib.xml8._unescape_xml, 'foo&bar;')
