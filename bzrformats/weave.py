    ...
    def check(self, progress_bar=None):
        ...
        for i in range(nv):
            ...
            if new_inc != self.get_ancestry(name):
                raise AssertionError(f"failed {new_inc} != {self.get_ancestry(name)}")
    ...
    def _imported_parents(self, other, other_idx):
        ...
        for parent_idx in other._parents[other_idx]:
            ...
            if parent_name not in self._name_map:
                ...
                raise WeaveError(
                    f"missing parent {{{parent_name}}} of {{{other._name_map[other_idx]}}} in {self!r}"
                )
...
def _reweave(wa, wb, pb=None, msg=None):
    ...
    logger.debug("combined parents: %r", combined_parents)
    order = tsort.topo_sort(combined_parents.items())
    logger.debug("order to reweave: %r", order)
    ...
    for idx, name in enumerate(order):
        ...
        if name in wa._name_map:
            ...
            if name in wb._name_map:
                ...
                if lines != lines_b:
                    logger.debug("Weaves differ on content. rev_id {%s}", name)
                    logger.debug("weaves: %s, %s", wa._weave_name, wb._weave_name)
                    import difflib
