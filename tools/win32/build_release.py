...
def brz():
    ...
    try:
        subprocess.call(
            ["brz", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        BRZ_EXE = "brz"
    except OSError:
        try:
            subprocess.call(
                ["brz.bat", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            BRZ_EXE = "brz.bat"
        except OSError:
...
def update_plugin(plugin_name):
    ...
    call_or_fail([brz(), "co", "-rtag:{}".format(tag_name), trunk_dir, release_dir])
    return release_dir
...
def build_installer():
    ...
    call_or_fail(["make", "PYTHON={}".format(PYTHON), "installer"], cwd=target)
